...
object Kafka09CsvTableExample {
  ...
  def main(args: Array[String]) {
    ...
    // The following snippet does not work! Because Csv() descriptor required connector.type="filesystem" only.
    //    tableEnvironment
    //      .connect(
    //        new Kafka()
    //          .version("0.9")
    ...
    val source = new Kafka09CsvTableSource(
      params.getRequired("input-topic"),
      params.getProperties,
      schema)

    source.setProctimeAttribute(TIMESTAMP_COL)
    tableEnvironment.registerTableSource(InputTable, source)
